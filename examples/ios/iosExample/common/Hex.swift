...
public final class Hex {
  ...
  static func toBytes(_ hex: String) -> [UInt8] {
    ...
    for i in stride(from: 0, to: length, by: 2) {
      let num = normalized.key_substring(from: i).key_substring(to: 2)
  ...
  static func hex(from bytes: [UInt8]) -> String {
    return Data(bytes: bytes).key_toHexString()
  }
  
  static func removePrefix(_ hex: String) -> String {
    if hasPrefix(hex) {
      return hex.key_substring(from: 2)
    } else {
      return hex
    }
  }
  ...
  static func isHex(_ string: String) -> Bool {
    if hasPrefix(string) {
      return isHex(string.key_substring(from: 2))
    }
...
public extension String {
  func key_toHexString() -> String {
    return data(using: .utf8)!.key_toHexString()
  }
  
  func key_fromHexString() -> String {
    if Hex.hasPrefix(self) {
      return key_substring(from: 2).key_fromHexString()
    }
  ...
  func key_dataFromHexString() -> Data? {
    if Hex.hasPrefix(self) {
      return key_substring(from: 2).key_dataFromHexString()
    }
    
    let length = count
    
    if length % 2 == 1 {
      return ("0" + self).key_dataFromHexString()
    }
