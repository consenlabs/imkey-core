...
public class BleByteUtil{
  public class func hex2Bytes(data:String) -> [UInt8]? {
  ...
  public class func bytes2Hex(data:[UInt8]) -> String{
    var res:String = ""
  ...
  public class func reserve(hexString:String) ->String{
    let array = hex2Bytes(data: hexString)
    let newArray = [UInt8](array!.reversed())
    return bytes2Hex(data: newArray)
  }
