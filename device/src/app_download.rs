use common::constants::{TSM_ACTION_APP_DOWNLOAD, TSM_RETURN_CODE_SUCCESS, TSM_END_FLAG};
use common::https;
use mq::message;
...
impl app_download_request {
    pub fn build_request_data(
        ...
        app_download_request {
            ...
            statusWord: None,
            commandID: String::from(TSM_ACTION_APP_DOWNLOAD),
    ...
    pub fn app_download(&mut self) -> Result<()> {
        loop {
            ...
            let mut response_data = https::post(TSM_ACTION_APP_DOWNLOAD, req_data)?;
            ...
            if return_bean._ReturnCode == TSM_RETURN_CODE_SUCCESS {
                ...
                return Err(ImkeyError::BAPP0006.into());
            }
        }
    }
}
