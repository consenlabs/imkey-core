...
impl app_update_request {
    ...
    pub fn app_update(&mut self) -> Result<()> {
        loop {
            ...
            let mut response_data = https::post(constants::TSM_ACTION_APP_UPDATE, req_data)?;
            ...
            if return_bean._ReturnCode == constants::TSM_RETURN_CODE_SUCCESS {
                ...
                match return_bean._ReturnCode.as_str() {
                    constants::TSM_RETURNCODE_APP_UPDATE_FAIL => Err(ImkeyError::IMKEY_TSM_APP_UPDATE_FAIL.into()),
