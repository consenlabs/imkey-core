...
class ViewController: UIViewController,BLEDelegate {
  ...
  func deviceDidConnect(_ uuid: String!, handler: Int, errorCode Error: Int) {
    handle = UInt(handler)
    Log.d("handle:\(handle)")
    Log.d("deviceDidConnect.. \(Error)")
  }
  
  func deviceDidDisconnect(_ uuid: String!, error Error: Int) {
    Log.d("deviceDidDisconnect \(Error)")
  }
...
//    Log.test_ffi()
  ...
  @IBAction func disconnect(_ sender: Any) {
    let res = BLE.shared().disConnect(handle: handle)
    if(res != 0){
  ...
  func showDeviceInfo(name:String,mac:String){
    ...
    DispatchQueue.global().async {//并行、异步
      do {
        let bleVersion = try Manager.getBLEVersion(handle: self.handle)
        let seid = try Manager.getSeid(handle: self.handle)
        //ramsize
        let battery = try Manager.getBatteryPower(handle: self.handle)
        let lifeTime = try Manager.getLifeTime(handle: self.handle)
...
extension ViewController:UITableViewDataSource,UITableViewDelegate{
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    do {
//      handle = 0
      let result = try BLE.shared().connect(address: device.address, handle: &handle, timeout: 12*1000)
