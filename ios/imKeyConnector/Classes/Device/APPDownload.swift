...
public class APPDownload:TSMReq{
  func download(handle:UInt,request:APPRequest)throws -> Response{
    var req = request
    req.commandID = Constants.appDownload
    
    guard let data = try? JSONEncoder().encode(req) else {
      throw SDKError.illegalArgument
    }
    let res = try HTTP.syncRequest(action: Constants.appDownload,from: data)
    ...
    if(response._ReturnData.nextStepKey == "end"){
      ...
      return try download(handle: handle,request: reRequest)
    }
  }
}
