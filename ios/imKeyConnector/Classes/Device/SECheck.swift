...
public class SECheck:TSMReq{
  func checkSE(handle:UInt, request:SERequest)throws -> Response{
    var req = request
    req.commandID = Constants.seSecureCheck
    
    guard let data = try? JSONEncoder().encode(req) else {
      throw SDKError.illegalArgument
    }
    let res = try HTTP.syncRequest(action: Constants.seSecureCheck,from: data)
    ...
    if(response._ReturnData.nextStepKey == "end"){
      ...
      let reRequest = SERequest(stepKey: response._ReturnData.nextStepKey!,
                                ...
                                deviceCert: "",
                                sn: "")
      return try checkSE(handle: handle, request: reRequest)
    }
  }
}
