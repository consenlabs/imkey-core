...
public class Bech32 {
  ...
  public static Bech32Data decode(final String str) throws AddressFormatException {
    ...
    for (int i = 0; i < str.length(); ++i) {
      char c = str.charAt(i);
      if (c < 33 || c > 126) throw new AddressFormatException(String.format("InvalidCharacter %c, %d",c, i));
      if (c >= 'a' && c <= 'z') {
        if (upper)
          throw new AddressFormatException(String.format("InvalidCharacter %c, %d",c, i));
        lower = true;
      }
      if (c >= 'A' && c <= 'Z') {
        if (lower)
    ...
    for (int i = 0; i < dataPartLength; ++i) {
      ...
      if (CHARSET_REV[c] == -1) throw new AddressFormatException(String.format("InvalidCharacter %c, %d",c, i + pos+1));
