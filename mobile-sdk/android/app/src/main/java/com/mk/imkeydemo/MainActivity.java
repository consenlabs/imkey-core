...
//        Api.check_update();
    ...
    private void showDeviceInfo(String name,String mac){
        ...
        es.execute(new Runnable() {
            @Override
            public void run() {
                final String bleVersion = DeviceApi.getBleVersion();
                final String seid = DeviceApi.getSeId();
                final String ramSize = DeviceApi.getRamSize();
                final String battery = DeviceApi.getBatteryPower();
                final String lifeCycle = DeviceApi.getLifeTime();
                final String sn = DeviceApi.getSn();
                final String firmwareVersion = DeviceApi.getFirmwareVersion();
                final String bleName = DeviceApi.getBleName();
                final Device.GetSdkInfoRes sdkInfo = DeviceApi.getSdkInfo();
            ...
            public void run() {
                try {
                    ...
                    if("unbound".equals(status)){
                        ...
                        if(!bindCode.isEmpty()){
                            toast("已绑定其他设备,重新绑定..");
                            String res = DeviceApi.bindAcquire(bindCode);
