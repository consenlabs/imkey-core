...
use common::utility::{hex_to_bytes, is_valid_hex, secp256k1_sign, sha256_hash};
...
impl TronSigner {
    ...
    pub fn sign_transaction(input: TronTxReq) -> Result<TronTxRes> {
        ...
        let key_manager_obj = KEY_MANAGER.lock().unwrap();
        let raw_data_sig = secp256k1_sign(&key_manager_obj.pri_key, &raw_data)?;
        data_pack.push(0x00);
        data_pack.push(raw_data_sig.len() as u8);
        data_pack.extend(raw_data_sig);
    ...
    pub fn sign(path: &str, data_pack: &[u8], hash: &[u8], sender: &str) -> Result<String> {
        ...
        let mut path_apdu_pack: Vec<u8> = vec![];
        path_apdu_pack.push(0x00);
        path_apdu_pack.push(path_signature.len() as u8);
        path_apdu_pack.extend(path_signature.as_slice());
        path_apdu_pack.push(0x01);
        path_apdu_pack.push(path.as_bytes().len() as u8);
        path_apdu_pack.extend(path.as_bytes());
        let msg_pubkey = Secp256k1Apdu::get_xpub(&path_apdu_pack);
...
mod tests {
    ...
    fn sign_transaction() {
        ...
        let input = TronTxReq{
            ...
            address: "TDQqJsFsStSy5fjG52KuiWW7HhJGAKGJLb".to_string(),
