use bitcoin::{Address, PublicKey, Network, TxOut, Transaction};
...
impl BtcTransaction{
    pub fn sign_transaction(&self, network : Network, path : &String) -> Result<(), BtcError>{
        ...
        //对签名原值进行SHA256
        let message_hash = digest::digest(&digest::SHA256, Vec::from_hex(sign_data).unwrap().as_slice());
        ...
        for utxo in &self.unspents {
            ...
            for index_number in index_number_vec {
                ...
                let test_chain_number = ChildNumber::from_normal_idx(index_number.parse().unwrap()).unwrap();
                pk = pk.ckd_pub(&bitcoin_secp, test_chain_number).unwrap();
                println!("{:?}", pk.to_string());
                println!("{:?}", pk.public_key.to_string().to_uppercase());
            }
            //验证地址
           let temp_address = Address::p2pkh(&PublicKey::from_str(pk.public_key.to_string().as_str()).unwrap(), network).to_string();
...
#[cfg(test)]
mod tests {
    use crate::transaction::{Transaction, Utxo, BtcTransaction};
